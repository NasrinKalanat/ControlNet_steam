...
class ImageNetVidDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, image_size=256, batch_size=1, len_seq=8, path="", path_weather="", path_scaler="",
        ...
        for fol in os.listdir(path):
            # load
            print(fol, path)
            images, temps, dates, weather, weather_label = self.load_data(path, fol)
            ...
            images, temps, dates, weather, weather_label = self.generate_many2many_data2(self.l_seq, images, temps,
                                                                                         dates, weather, weather_label)
            if images.shape[0] > 0:
                # split to train, val, test
                images, labels, dates, weather, weather_label = self.data_split(images, temps, dates, weather,
                                                                                weather_label, self.phase, mask_frac)
        ...
        if normalize_flag:
            ...
            # weather_label = self.wBinarizer.fit_transform(
            #     self.weather_label.reshape(self.weather_label.shape[0] * self.weather_label.shape[1]))
            # self.weather_label = weather_label.reshape(self.weather_label.shape[0], self.weather_label.shape[1], -1)
            # joblib.dump(self.wBinarizer, os.path.join(path_scaler, "weatherlabel_scaler_" + phase))
            # # print(self.wBinarizer.classes_)

            self.timetransformer = MinMaxScaler()
            dates = self.timetransformer.fit_transform(
                self.dates.reshape(self.dates.shape[0] * self.dates.shape[1], -1))
    ...
    def load_data(self, path, fol):
        ...
        with open(self.path_weather + "response.jsonl", 'r') as weatherlabelfile:
            for line in weatherlabelfile.readlines():
                ...
                w_jdata = json.loads(json.loads(line)[1]["choices"][0]["message"]["content"])[
        ...
        return images, temps, dates, data[weatherfile.columns].drop(columns=["weather_label"], axis=1).values, data[
            "weather_label"].values
    ...
    def data_split(self, t_x, t_y, t_d, weather, weather_label, phase, mask_frac):
        ...
        # Define split sizes
        train_size = int(0.6 * t_x.shape[0])
        ...
        if phase == "pretrain":
            ...
            if not mask_frac:
                ...
                weather_label = weather_label[
                                round(weather_label.shape[0] * 0.675):round(weather_label.shape[0] * 0.875)]
            else:
            ...
            if not mask_frac:
                ...
                weather_label = weather_label[
                                round(weather_label.shape[0] * 0.375):round(weather_label.shape[0] * 0.675)]
            else:
    ...
    def __getitem__(self, id):
        ...
        imgs = np.zeros((self.l_seq + 1, 3, self.image_size, self.image_size), dtype=torch.float32)
        imgs_random = np.zeros((self.l_seq + 1, 3, self.image_size, self.image_size), dtype=torch.float32)
